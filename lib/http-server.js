...
var HTTPServer = exports.HTTPServer = function (options) {
  ...
  var dir = options.quiet ? (function() {}) : console.dir.bind(console);
  ...
  portfinder.getPort(function (err, port) {
    if (err) { log("Error, no open ports available."); dir(err); return; }
    ...
    this.wss.broadcast = function broadcast(data) {
      for(var i in this.clients) {
        try { this.clients[i].send(data); } catch (ex) { log('error sending to client.'); dir(ex); }
      }
    };
    ...
    watcher.on('ready', function() { log('Scanned working directory. ready for changes..'); watcher_ready = true; });
    ...
    watcher.on('change', function(filename) {
      if (!watcher_ready) return;
      log('change detected! telling clients to reload. ['+filename+']');
